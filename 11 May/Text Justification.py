# Given an array of strings words and a width maxWidth, format the text such that each line has exactly maxWidth characters and is fully (left and right) justified.

# You should pack your words in a greedy approach; that is, pack as many words as you can in each line. Pad extra spaces ' ' when necessary so that each line has exactly 
# maxWidth characters.

# Extra spaces between words should be distributed as evenly as possible. If the number of spaces on a line does not divide evenly between words, 
# the empty slots on the left will be assigned more spaces than the slots on the right.

# For the last line of text, it should be left-justified and no extra space is inserted between words.

class Solution:
    def fullJustify(self, words, maxWidth):
        line = []
        lwords = 0
        output = []
        for word in words:
            if (len(word) + lwords + len(line) > maxWidth):
                nspaces = maxWidth - lwords
                minspc = nspaces // ((len(line) - 1) or 1)
                remainder = nspaces % ((len(line) - 1) or 1)
                for i in range((len(line)-1) or 1):
                    nspc = minspc
                    if (i < remainder):
                        nspc += 1
                    line[i] += ' ' * nspc
                output.append(''.join(line))
                line = []
                lwords = 0
            line.append(word)
            lwords += len(word)
        trailingSpace = maxWidth - lwords - len(line) + 1
        output.append(' '.join(line) + ' ' * trailingSpace)
        return output
